...
#from free_energy.md import MolecularDynamics
from mdclass import MolecularDynamics
from gpaw.lcao.tools import MASTER, rank
from ase.db import *
...
class LangevinC(MolecularDynamics):
	...
	def __init__(self, atoms, timestep, temperature, friction, fixcm=True, constr=False,
		index=0, sol_index=3, list_sub=None, list_sol=None, calc2=None, mix=0, MM2=0, trajectory=None, logfile=None, 
                loginterval=1, communicator=gpaw_world, db=None):
	...
	def step(self, f):
	    ...
	    # Update the forces; Here the potential energy and forces of PES2 are calculated:
	    f_pes1 = atoms.get_forces()

            if self.calc2 is not None:
                mix  = self.mix
                MM2 = self.MM2
                sys2 = atoms.copy()
                sys2.set_initial_magnetic_moments(MM2)
                sys2.set_calculator(self.calc2)
                f_pes2 = sys2.get_forces()
            
                f = f_pes1 * (1.0 - mix) + f_pes2 * mix
		...
		#dbmol = atoms.copy()
		#dbmol.set_calculator()
                if self.calc2 is not None:
		    #c.write(atoms=dbmol, mix=self.mix, Ecalc1=E1, Ecalc2=E2, T=T)
		    c.write(atoms=atoms, mix=self.mix, Ecalc1=E1, Ecalc2=E2, T=T)
                else:
		    #c.write(atoms=dbmol, mix=self.mix, Ecalc1=E1, T=T)
		    c.write(atoms=atoms, mix=self.mix, Ecalc1=E1, T=T)
	    return f
