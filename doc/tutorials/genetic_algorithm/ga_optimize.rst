...
``ase.optimize.genetic_algorithm`` which includes all classes needed
for the optimizer. 
...
list or directly to Lasse Vilhelmsen at lassebv@phys.au.dk.
...
storage the main script can be execute multiple times to generate new
candidates.
...
:math:`\text{Ag}_2\text{Au}_2` cluster on a Au(111) surface using the
EMT optimizer.
